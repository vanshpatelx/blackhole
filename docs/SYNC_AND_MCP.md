...
- **Permanent addresses (v0.2.1):** each install enrolls once with a small Cloudflare Worker (`server/`) that creates a Cloudflare Tunnel and a DNS record like `mcp-a7f3.getblackhole.app`, then runs that tunnel locally. Requests flow from the assistant to Cloudflare's edge and down the user's own tunnel; the Worker only issues credentials and never sees task data. Cloudflare terminates TLS at its edge, so it *could* read traffic in transit; Tailscale Funnel remains the option where nobody in the middle can.
- **Zero-install tunnel (v0.2.1):** turning on public access downloads a pinned, checksum-verified `cloudflared` into Application Support, so the common path (claude.ai / ChatGPT) needs no Homebrew or Terminal.
